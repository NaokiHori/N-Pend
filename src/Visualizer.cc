...
Visualizer::Visualizer(const std::vector<class Pendulum> pendulums, const std::array<int, 4> window_position, const std::array<unsigned int, 2> pendulum_origin, std::vector<const std::string> colors){
  /*
   * constructor
   */
  const int n_p = pendulums.size();
  const int N = 3;
  int left, top;
  int screen;
  Colormap cmap;
  ...
  mcolors=(XColor*)malloc(sizeof(XColor)*n_p);
  for(int n=0; n<n_p; n++){
    XAllocNamedColor(display, cmap, colors[n].c_str(), &(mcolors[n]), &(dummy));
  }
}

void Visualizer::update(std::vector<Pendulum> pendulums, const double time){
  // clear window before drawing
  XClearWindow(display, window);
  for(int pidx=0; pidx<pendulums.size(); pidx++){
    // load necessary info to draw from pendulum class
    const int N                       = pendulums[pidx].get_N();
    const std::vector<double> thetas  = pendulums[pidx].get_thetas();
    const std::vector<double> lengths = pendulums[pidx].get_lengths();
    ...
    // draw masses
    XSetForeground(display, gc, mcolors[pidx].pixel);
